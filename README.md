...
```
#### Gravando em um Pendrive (Bare-Metal)
```bash
...
qemu-system-aarch64 -M virt -cpu cortex-a57 -nographic -kernel target/LuusOS.bin
```
#### Flashing to a USB Drive (Bare-Metal Deployment)
```bash
sudo dd if=build/luusos.iso of=/dev/sdX bs=4M && sync
```
## 📦 🇧🇷 Download Rápido / 🇺🇸 Quick Download (Via Releases)
### Linux / Termux Terminal:
```bash
# PT: Baixar a imagem ISO estável e pronta para boot
# EN: Download the bootable, production-ready ISO image
...
# PT: OU capturar apenas o binário estático do Kernel
# EN: OR fetch the freestanding raw Kernel binary output
